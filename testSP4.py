...
plt.plot(data[:,0], data[:,1], "ro-", label="N=100")

data = np.loadtxt(fname2, delimiter=',')
plt.plot(data[:,0], data[:,1], "gx-", label="N=200")

data = np.loadtxt(fname3, delimiter=',')
plt.plot(data[:,0], data[:,1], "b^-", label="N=300")

data = np.loadtxt(fname4, delimiter=',')
plt.plot(data[:,0], data[:,1], "cd-", label="N=400")

data = np.loadtxt(fname5, delimiter=',')
plt.plot(data[:,0], data[:,1], "mp-", label="N=500")

plt.legend(loc="lower right")

title="/Users/iwaitoshiya/Desktop/mcSPtest_UNITChange.png"
plt.savefig(title)
